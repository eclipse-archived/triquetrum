...
public class TaskProcessingTest {
  ...
  /**
   * This is the simplest illustration of a working case for processing a task : - there's a (mock) implementation of a TaskProcessingService that accepts to
   * process the task - we don't care about results yet, we just want to make sure the Task status is FINISHED afterwards.
   */
  @Test
  ...
  /**
   * This test exploits the features of CompletableFuture to process 2 concurrent subtasks asynchronously, and set the MainTask to finished when both subtasks
   * are done.
   */
  @Test
  ...
  public void testMainAndSubTaskSequenceProcessing3() {
    ...
    // So even though we don't really need a main task for this unit test, we show the construction here as a typical code example.
    Task mainTask = triqFactory.createTask(null, "testInitiator", "testType", "testCorrelationId", "testExternalRef");
    triqFactory.createTask(mainTask, "testInitiator", "testType", "testCorrelationId", "testExternalRef");
    triqFactory.createTask(mainTask, "testInitiator2", "testType", "testCorrelationId2", "testExternalRef2");

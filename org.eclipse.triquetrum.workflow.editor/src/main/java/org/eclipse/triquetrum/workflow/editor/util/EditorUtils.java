...
public class EditorUtils {
  ...
  /**
   * Current color-treatment is based on storing a comma-separated string of rgba values.
   * RGBA values must be integers in the range [0-255].
   * <p>
   ...
   * </p>
   * Remark that Graphiti's Color does not support setting opacity/alpha.
   * So its value is currently silently ignored in Triq.
   *
   * @param colorStr a comma-separated rgba value list. It can be partially specified, e.g. just rg or so.
   *  The non-specified elements will be set with defaults of 0 for the color components and 1 for the alpha.
   * @return
   *
   * @see RGBA
   * @see Color
...
//    int alpha = 1;
  ...
  public static Diagram getSelectedDiagram() {
    ...
    if (sel != null && sel instanceof IStructuredSelection) {
      ...
      if (res instanceof DiagramEditPart) {
        result = (Diagram) ((DiagramEditPart) res).getModel();
      }
    }
    return result;
  }

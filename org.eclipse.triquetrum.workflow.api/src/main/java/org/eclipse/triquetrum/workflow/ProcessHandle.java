...
public interface ProcessHandle extends Serializable {
  ...
  /**
   * For context-aware executions, this can be used to retrieve the <code>ProcessManager</code> from the <code>ProcessManagerService</code> if needed. <b>Remark
   * that such retrieval can be a heavy operation and should only be attempted when really necessary.</b> <br/>
   * For the rare process executions without assigned <code>Context</code>s, this returns an id that can be used to uniquely identify the execution in any
   * related actions, e.g. to obtain execution logs, pause/resume it etc.
   *
